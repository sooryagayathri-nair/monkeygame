...
function setup(){
 createCanvas(windowWidth,windowHeight);
  ...
  monkey = createSprite(80,height-250,10,10);
  monkey.scale = 0.12;
  ...
  invisiGround = createSprite(width/2,height-110,600,7);
...
function draw(){
  ...
  if (gameState === PLAY){
    ...
    if(keyDown("space")&&monkey.y >= height-160) {
      monkey.velocityY = -13; 
    }
...
function bananas(){
  if (frameCount%80 === 0){
    ...
    banana = createSprite(800,height-270, 50, 50 )
...
function obstacles(){
  if (frameCount%200 === 0){
    ...
    obstacle = createSprite(620,height-137,50,50);
    obstacle.addAnimation("rock", obstacleImage);
    obstacle.setCollider("circle", 0, 0, 180);
    obstacle.scale = 0.13 ;
    obstacle.velocityX = -(7+score*1.5/100);
